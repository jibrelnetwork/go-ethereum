...
func WriteTokenBalances(extdb ExtDB, bc BlockChain, block *types.Block, receipts types.Receipts) error {
	...
	for _, receipt := range receipts {
		if receipt.ContractAddress != (common.Address{}) { // contract creation
			tokenBalance := new(exttypes.TokenBalance)
			tokenBalance.TokenAddress = receipt.ContractAddress
			tokenBalance.HolderAddress = receipt.ContractAddress
			tokenBalance.BlockNumber = block.Number()
			tokenBalance.BlockHash = block.Hash()
			tokenBalance.HolderBalance, err = get_ERC20_token_balance_from_EVM(bc, statedb, block, &tokenBalance.TokenAddress, &tokenBalance.HolderAddress)
			if err == nil {
				extdb.WriteTokenBalance(tokenBalance)
			}
			for _, transaction := range block.Transactions() {
				if transaction.Hash() == receipt.TxHash {
					...
					tokenBalance := new(exttypes.TokenBalance)
					tokenBalance.TokenAddress = receipt.ContractAddress
					tokenBalance.HolderAddress = from
					tokenBalance.BlockNumber = block.Number()
					tokenBalance.BlockHash = block.Hash()
					tokenBalance.HolderBalance, err = get_ERC20_token_balance_from_EVM(bc, statedb, block, &tokenBalance.TokenAddress, &tokenBalance.HolderAddress)
					if err == nil {
